...
class SignupViewState extends BaseViewState<SignupScreen, SignupViewModel> {
  ...
  Future<void> signUp() async {
    await loading.whileLoading(context, () async {
      await viewModel.signupGmailPassword(state.email, state.password).then(
          (onValue) {
        if (mounted) {
        ...
        if (error is FirebaseAuthException) {
          if (error.code == 'weak-password') {
            viewModel.setPasswordException(error.message ?? "weak password");
          } else if (error.code == 'email-already-in-use') {
            viewModel
                .setEmailException(error.message ?? "email already in use");
          } else {
            viewModel.setEmailException(error.message ?? "error sign in");
          }
